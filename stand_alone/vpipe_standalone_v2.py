...
def get_eop(geo_path):
    if os.path.exists(geo_path+'usno_finals.erp'):
        ...
        if age<12: pass
            ...
            print 'EOP---> Use latest downloaed erp file'
        #--- ZB
...
def new_uvflg(indata,timeran,bif,eif,bchan,echan,antennas,outfg):
    uvflg        = AIPSTask('UVFLG')
    uvflg.indata = indata
    uvflg.opcode = 'FLAG'
...
def run_aclip(indata,infg,outfg,gainuse,ant,ifnum,pol,vmax,near):
    ...
    aclip.aparm[7]    = near    #flag surrounding channels
    aclip.go()
...
def loadindx(filepath,filename,outname,outclass,
   ...
   # if aipsver!='31DEC09':
    ...
    if data.exists():
        ...
        mprint('##############################',logfile)
        mprint('No need reindexd!',logfile)
        pass
        #data.clrstat()
        #data.zap()
        #mprint('##############################',logfile)
...
def check_sncl(indata,sn,cl,logfile):
    clnum = indata.table_highver('AIPS CL')
    ...
    if indata.table_highver('AIPS CL')>cl:
        if clnum>10:
            issue = input('I found that your original CL table version is '+str(clnum)+', which means you have done fairly a lot of work before. Are you sure you want to continue the progress which may erase your previous saved CL tables?(type "y" for continue anyway, type "n" to stop)')
            if issue == 'yes' or issue == 'y':
                pass
            elif issue == 'no' or issue =='n':
                print 'Shutting down the progress'
                sys.exit()
            else:
                print 'Confusing answer, will take it as no'
                sys.exit()
...
##############################################################################
#
def run_bpass(indata, bandcal,gainuse,flagver,refant,logfile):
    if bandcal==['']:
        mprint('No Bandpass calibrator selected.', logfile)
        sys.exit()
    ...
    bpass.refant       = refant
    bpass.solint       = -1
    bpass.bpassprm[4]  = 0 # store both phase and amp
    bpass.bpassprm[5]  = 1 #not devide by channel 0
    bpass.bpassprm[9]  = 1 #interpolate over flagged channels
    bpass.bpassprm[10] = 6 #amp power normalization on all chanelss
    print type(bpass.bpassprm[1])
    print type(bpass.ichansel[1])
    bpass.ichansel[1]     = [None,1,256,1,0] 
    #bpass.ichansel[2]     = [None,1,90,1,4] #specially for IF4 in VLBA Lband
    bpass.outver       = 1
    bpass.input()
    bpass.go()
...
def list_sncl(indata,inext,inver,sources,listparm):
    listr=AIPSTask('LISTR')
    listr.indata   = indata
    listr.optype   = 'GAIN'
    ...
    if listparm == 'phase':
        ...
        listr.dparm[1] = 11
    if sources == '':
        filename =  inext+str(inver)+'-'+listparm[0:2]+'allsources.listr'
    elif len(sources) <=1:
        filename=  inext+str(inver)+'-'+listparm[0:2]+'-'+sources[0][0:5]+'-'+'.listr'
    elif len(sources) >=2:
        filename=  inext+str(inver)+'-'+listparm[0:2]+'-'+sources[0][0:5]+'-'+sources[1][0:5]+'.listr'
    listr.outprint = 'PWD:'+filename
...
def run_acscl(indata,flagver,gainuse,doband,bpver):
    acscl           = AIPSTask('ACSCL')
    acscl.indata    = indata
    acscl.timer[1:] = [0]
    acscl.flagver   = flagver
    acscl.doband    = doband
    acscl.docalib   = 1
    acscl.gainuse   = gainuse
    acscl.bpver     = bpver
    acscl.solint    = -5
    #acscl.ichansel[1:]  = chansel
    acscl()  
##############################################################################
#
...
def runapcal(indata, tyver, gcver, snver, dofit, opcode):
    ...
    apcal.snver      = snver
    apcal.opcode     = opcode
    #apcal.bif        = 0  #for data with wide or multi bands but in the same file
    #apcal.eif        = 0
    ant              = get_ant(indata)
    apcal.dotv       = -1
    if antname =='VLBA':
        if indata.table_highver('AIPS WX') > 0:
            apcal.inver      = 1 # use WX table 1
        else:
            apcal.inver      = 0
            #apcal.calin         = 'PWD:'+outname[0]+'.weather' # if no wx table but can find it in VOBS. make the table and name it as outname.weather in the pwd directory. try VLOG but failed. Maybe need to update the AIPS version
        for i in ant:
            apcal.dofit[i]  = dofit
    elif antname == 'LBA':
        for i in ant:
...
##############################################################################
#
def fringecal(indata, fr_image, nmaps, gainuse, refant, refant_candi, calsource,solint,smodel, doband, bpver, dpfour,apfive,ftrate,dwin,rwin):
    fringe             = AIPSTask('FRING')
    if fr_image.exists():
    ...
    fringe.aparm[1:]   = [2, 0, 1, 0, apfive, 0, 0, 0, 1] #change if needed
    fringe.dparm[1:]   = [0, dwin, rwin, 0]
    fringe.dparm[4]    = dpfour
    fringe.dparm[8]    = 0 # zeroing rate, delay, phase?
    fringe.dparm[9]    = ftrate # supressing rate(1) or not(0)?
    fringe.nmaps       = nmaps
    fringe.snver       = 0
    fringe.gainuse     = gainuse
...
def fringecal_ini2(indata, refant, refant_candi, calsource, gainuse, flagver, solint, doband, bpver, apfive, ftrate,delawin, ratewin):
    fringe             = AIPSTask('FRING')
    fringe.indata      = indata
    fringe.refant      = refant
    fringe.docal       = 1
    ...
    fringe.search[1:]  = refant_candi
    fringe.solint      = solint
    fringe.aparm[1:]   = [3, 0, 1, 0, apfive, 0, 0, 0, 1]
    fringe.dparm[1:]   = [0, delawin, ratewin, 0]
    #fringe.dparm[4]   = dpfour
    fringe.dparm[8]    = 0
    fringe.dparm[9]    = ftrate
    fringe.gainuse     = gainuse
...
def fringecal_ini(indata, refant, refant_candi, calsource, gainuse, flagver, solint, doband, bpver, apfive):
    fringe             = AIPSTask('FRING')
    fringe.indata      = indata
    fringe.refant      = refant
    fringe.docal       = 1
...
def calib_1(indata,fr_image,smode,gainuse,refant,snout,doband,bpver,calsour,flagver,solint):
...
##############################################################################
#
def gcal_app(indata,matx,cluse,pol):
    clcor             = AIPSTask('CLCOR')
    clcor.indata      = indata
...
def runsncor(indata,timer,inver):
    sncor              = AIPSTask('SNCOR')
    sncor.indata       = indata
    sncor.timerang[1:] = timer
    sncor.opcode       = 'ZRAT'
    sncor.bif         = 0
    sncor.eif         = 0
    sncor.snver        = invser
...
##############################################################################
#
def runpossm(indata, calsource='', ant_use=[0], doband=-1, bpver=1, bchan=1, echan=0, gainuse=1, flagver=0, stokes= 'HALF', nplot=9):
...
def runsnplt(indata,inver=1,inex='cl',sources='',optype='phas',nplot=4,timer=[]):
    ...
    lwpla.indata = indata
    if sources == '':
        filecode = outname[0]+'-'+inex+str(inver)+'-'+optype
    else:
        filecode=  outname[0]+'-'+inex+str(inver)+'-'+optype+'-'+sources[0]+'-'+sources[1]
    lwpla.outfile = 'PWD:'+filecode[:32]+'.snplt'
    lwpla.plver = 1
    lwpla.inver = 200
    filename = filecode[:32]+'.snplt'
    print filename
    if os.path.exists(filename)==True:
        os.popen('rm '+filename)
    lwpla.go()
...
#        splt_data.zap()
    if fittp == 1:
        ...
        if splt_data.exists():
            mprint('Writing out calibrated and splitted uv-data for '+targets[0],logfile)
            fittp.go()
        else:
            mprint('No calibrated and splitted uv-data for '+targets[0],logfile)
        if os.path.exists(outname[0]+'/'+fitname):
            os.popen(r'rm '+outname[0]+'/'+fitname)
        if os.path.exists(fitname):
            os.popen(r'mv '+fitname+' '+outname[0]+'/')
...
#zyk++chk
def chk_sn_cl(indata,snchk,clchk,source_chk,cl_trange,bpv):
    if bpv == 0:
        doband = -1
    else:
        doband = 1
    #runsnplt(pr_data,inver=snchk,inex='SN',sources=source_chk,optype='DELA',nplot=4,timer=[])
    #runsnplt(pr_data,inver=snchk,inex='SN',sources=source_chk,optype='RATE',nplot=4,timer=[])
    possmplot(pr_data,sources=source_chk,timer=chk_trange,gainuse=clchk,flagver=flagver,stokes='HALF',nplot=9,bpv=bpv,ant_use=[0],cr=1) 
...
##############################################################################
if 'ld_fr2_fringe_flag' in locals() and globals(): pass
else: ld_fr2_fringe_flag = 0
if 'resi_phcal_flag' in locals() and globals(): pass
else: resi_phcal_flag = 0
if 'sn_rotate_flag' in locals() and globals(): pass
else: sn_rotate_flag = 0
if 'app_rotate_flag' in locals() and globals(): pass
else: app_rotate_flag = 0
if 'splt_3_flag' in locals() and globals(): pass
else: splt_3_flag = 0
if 'ld_fr_fringe_flag' in locals() and globals(): pass
else:ld_fr_fringe_flag  = 0
if 'do_fr_fringe_flag' in locals() and globals(): pass
else:do_fr_fringe_flag  = 0
if 'do_calib_1_flag' in locals() and globals(): pass
else:do_calib_1_flag    = 0
if 'check_delay_rate' in locals() and globals(): pass
else:check_delay_rate   = 0
if 'split_2_flag' in locals() and globals(): pass
else:split_2_flag       = 0
if 'do_uvshift_flag' in locals() and globals(): pass
else:do_uvshift_flag       = 0
if 'first_cal_flag' in locals() and globals(): pass
else:first_cal_flag       = 0
if 'new_amp_flag' in locals() and globals(): pass
else:new_amp_flag       = 0
if 'apcal_flag' in locals() and globals(): pass
else:apcal_flag      = 0
if 'pang_flag' in locals() and globals(): pass
else:pang_flag       = 0
if 'do_fringe_flag' in locals() and globals(): pass
else: do_fringe_flag   = 0
if 'plot_first_run' in locals() and globals(): pass
else:plot_first_run   = 0
if 'split_1_flag' in locals() and globals(): pass
else:split_1_flag   = 0 

if 'pr_fringe_flag' in locals() and globals(): pass
else:pr_fringe_flag   = 0
...
if 'do_gaincor_flag' in locals() and globals(): pass
else: do_gaincor_flag = 0
...
if 'fr_nm' in locals() and globals(): pass
else: fr_nm = ''
if 'fr_cls' in locals() and globals(): pass
else: fr_cls = ''
if 'fr_dsk' in locals() and globals(): pass
else: fr_dsk = defdisk
if 'fr_sq' in locals() and globals(): pass
else: fr_sq = 1
...
if 'RDBE_check_flag' in locals() and globals():pass
else: RDBE_check_flag = 0
...
for i in range(n):
    ...
    if dtsum_flag==1:
        ...
        if listr_flag==1:
            runlistr(data[i])
        if get_key_flag==1 and antname == 'VLBA':
...
if geo_prep_flag>0:
    ...
    if geo_data.header['telescop']=='EVN': # only TECOR for EVN, no eops
        if geo_prep_flag==1:
            runTECOR(geo_data,year,doy,num_days,3,TECU_model)
        elif geo_prep_flag==2:#skip tecr
...
if dofit_flag==1 and do_geo_block==1:
    ...
    if delzn_flag==1:
        ...
        if sx_geo==False:
            ...
            os.popen(fit_path+'fit_geoblocks > fit_geoblocks.prt','w')
            ...
            os.popen(fit_path+'fit_geoblocks > fit_geoblocks_tropos.prt','w')
...
mprint('############################',logfile)
mprint('Data flaging before apcal',logfile)
mprint('############################',logfile)

if quack_flag==1:#deal with fgv2
    if data[0].table_highver('AIPS FG')>=2:
        data[0].zap_table('AIPS FG',2) 
    else:
...
if clip_flag >= 1:
    if data[0].table_highver('AIPS FG')>=2:
        #data[0].zap_table('AIPS FG',clip_outfg)
        infg=infg
    else:
        infg=1
    run_aclip(data[0],infg,clip_outfg,3,0,0,'',2.5,0)
    if clip_flag ==2:#tbd
        for i in range(antnum):
            for j in range(ifnum):
                run_aclip(data[0],infg,clip_outfg,3,i+1,j+1,'RR',clpmx[0][j+4*i],clpmx[2][j+4*i])
                run_aclip(data[0],infg,clip_outfg,3,i+1,j+1,'LL',clpmx[1][j+4*i],clpmx[2][j+4*i])


if man_uvflg_flag == 1:
    if data[0].table_highver('AIPS FG')>=3:
        ...
        for i in range(len(fgbchan)):
            ...
            print fgbchan[i]
            print fgechan[i]
            new_uvflg(data[0],fgtimer,fgbif[i],fgeif[i],fgbchan[i],fgechan[i],fgantennas[i],outfg)
###################################################################
...
for i in pr_data_nr:
    ...
    if first_cal_flag==1:#include accor and pang
        check_sncl(pr_data,0,3,logfile)
        runaccor(pr_data)
        runclcal(pr_data, 1, 3, 4, 'self', 1, refant)
        mprint('######################',logfile)
        mprint('finish accor',logfile)
        mprint('######################',logfile)
        runpang2(pr_data)
        mprint('######################',logfile)
    ...
    if pr_fringe_flag==1:
        ...
        mprint('######################',logfile)
        check_sncl(pr_data,1,5,logfile)
        man_pcal(pr_data, refant, mp_source, mp_timera,5,logfile, dpfour)
        runclcal2(pr_data,2,5,6,'2pt',0,refant,[0],mp_source,'')
    if new_amp_flag==1: #TODO, optimize bandpass method and improve this 
        check_sncl(pr_data, 2, 6,logfile)
        if pr_data.table_highver('AIPS BP') >= 1:
            pr_data.zap_table('AIPS BP', -1)
            run_bpass(pr_data, bandcal,6,flagver,refant,logfile)
        #run_bpass(indata, bandcal,gainuse,flagver,refant,logfile)
        else:
            run_bpass(pr_data, bandcal,6,flagver, refant,logfile)
            run_acscl(pr_data,flagver,6,2,1)
            #run_acscl(indata,flagver,gainuse,doband,bpver)
            #runclcal2(pr_data,3,6,7,'2pt',0,refant,[0],'','')
        runclcal(pr_data, 3, 6, 7, 'SELF', 1, refant)
    if new_amp_flag==2: #only use aclip and acscl, no bandpass used,iteration not ready; This may not be good.
        check_sncl(pr_data, 2, 6,logfile)    
        for i in range(6):
            if i <=0:
                run_aclip(data[0],2,flagver,6,0,0,'',2.5,8)
      #      run_bpass(data[0], bandcal,6,flagver,refant,logfile)
                run_acscl(data[0],flagver,6,2,-1)
            else:
                pass
        runclcal(pr_data, 3, 6, 7, 'self', 1, refant)
    if new_amp_flag==3: #no acscl used
        check_sncl(data[0], 2, 6,logfile)
        run_bpass(data[0], bandcal,6,flagver,refant,logfile)
        runtacop(data[0],data[0], 'CL', 6, 7, 1)
        runtacop(data[0],data[0], 'SN', 2, 3, 1)
    if apcal_flag == 1:
        check_sncl(pr_data,3,7,logfile) 
        runapcal(pr_data, tyver, 1, 4, dofit, 'GRID') #generate sn4
        runclcal(pr_data, 4, 7, 8, '',  1, refant)
...
if bandcal==['']:
    ...
    if do_fringe_flag == 1:
        ...
        mprint('######################',logfile)
        check_sncl(pr_data,4,8,logfile)
        doband = -1
        fringecal_ini(pr_data,refant, refant_candi, calsource[0],8,flagver,solint,doband,bpver,apfive)
        fringecal_ini(pr_data,refant, refant_candi, p_ref_cal[0],8,flagver,solint,doband,bpver,apfive)
    #fringecal_ini(pr_data,refant, refant_candi, p_ref_cal,7,flagver,solint,-1,0,apfive)
        runclcal2(pr_data,5,8,9,'ambg',-1,refant,[0],calsource,calsource)
        runclcal2(pr_data,6,8,10,'ambg',1,refant,[0],p_ref_cal[0],targets)
    if do_fringe_flag == 2:
        mprint('######################',logfile)
        mprint('Begin first fringe',logfile)
        mprint('######################',logfile)
        check_sncl(pr_data,4,8,logfile)
        fringecal_ini(pr_data,refant, refant_candi, calsource[0],8,flagver,solint,-1,0,apfive)
        fringecal_ini(pr_data,refant, refant_candi, targets,8,flagver,solint,-1,0,apfive)
        runclcal2(pr_data,5,8,9,'ambg',-1,refant,[0],calsource,calsource)
        runclcal2(pr_data,6,8,10,'ambg',-1,refant,[0],target,targets)
#fringecal_ini(indata, refant, refant_candi, calsource, gainuse, flagver, solint, doband, bpver,apfive)
    if do_fringe_flag == 3:
        mprint('######################',logfile)
        mprint('Do fringe for special cases',logfile)
        mprint('######################',logfile)
        check_sncl(pr_data,8,cluse+1,logfile)
        fringecal_ini(pr_data,refant, refant_candi, calsource[0],cluse+1,flagver,solint,-1,0,apfive)
        fringecal_ini(pr_data,refant, refant_candi, targets,cluse+1,flagver,solint,-1,0,apfive)
        runclcal2(pr_data,4,cluse+1,cluse+2,'ambg',-1,refant,[0],calsource,calsource)
        runclcal2(pr_data,5,cluse+1,cluse+3,'ambg',-1,refant,[0],target,targets)
    ...
    if plot_first_run == 1:
        ...
        runsnplt(pr_data,inver=6,inex='SN',sources=targets,optype='DELA',nplot=4,timer=[])
        runsnplt(pr_data,inver=6,inex='SN',sources=targets,optype='RATE',nplot=4,timer=[])
        possmplot(pr_data,sources=p_ref_cal[0],timer=chk_trange,gainuse=10,flagver=flagver,stokes='HALF',nplot=9,bpv=0,ant_use=[0],cr=1)
        possmplot(pr_data,sources=p_ref_cal[0],timer=chk_trange,gainuse=10,flagver=flagver,stokes='HALF',nplot=1,bpv=0,ant_use=[0],cr=0)
        possmplot(pr_data,sources=p_ref_cal[0],timer=chk_trange,gainuse=10,flagver=flagver,stokes='HALF',nplot=9,bpv=1,ant_use=[0],cr=1)
        mprint('######################',logfile)
        mprint(get_time(),logfile)
        mprint('######################',logfile)
    if do_band_flag==1:
        check_sncl(pr_data, 6, 10,logfile)
        if pr_data.table_highver('AIPS BP') >= 1:
            pr_data.zap_table('AIPS BP', -1)
            do_band(pr_data, bandcal,9,1, logfile)
        else:
            do_band(pr_data, bandcal,9,1, logfile)
        possmplot(pr_data,sources=p_ref_cal[0],timer=chk_trange,gainuse=10,flagver=0,stokes='HALF',nplot=9,bpv=1,ant_use=[0],cr=1)
        possmplot(pr_data,sources=bandcal[0],timer=possm_scan,gainuse=9,flagver=0,stokes='HALF',nplot=9,bpv=1,ant_use=[0],cr=1)
...
if split_1_flag == 1:
    ...
    run_split2(cont_data, calsource[0], 9, 'v1cal', doband, bpver, flagver, split_seq)
    run_split2(cont_data, p_ref_cal[0], 10, 'v1ref', doband, bpver, flagver,split_seq)
    if len(p_ref_cal)>=2:
        run_split2(cont_data, p_ref_cal[1], 10, split_outcl, doband, bpver, flagver,split_seq)
        #run_fittp_data(source, split_outcl, defdisk, logfile)
        run_split2(cont_data, target[0], 10, 'v1tar', doband, bpver, flagver,split_seq)
if split_1_flag == 2:
    check_sncl(cont_data, 6, 10,logfile)
    run_split2(cont_data, calsource[0], 9, split_outcl, doband, bpver, flagver,split_seq)
    run_split2(cont_data, p_ref_cal[0], 10, 'BKREF', doband, bpver, flagver,split_seq)
    run_split2(cont_data, target[0], 10,'SELF', doband, bpver, flagver,split_seq)
    #if len(p_ref_cal)>=2:
    #run_split2(cont_data, p_ref_cal[1], 9, split_outcl, doband, bpver, flagver)
if split_1_flag == 3:
    doband =-1
    check_sncl(cont_data, 15, cluse+3,logfile)
    run_split2(cont_data, calsource[0], cluse+2, split_outcl, doband, bpver, flagver,split_seq)
    run_split2(cont_data, p_ref_cal[0], cluse+3, 'BKREF2', doband, bpver, flagver,split_seq)
    run_split2(cont_data, target[0], cluse+3,'SELF2', doband, bpver, flagver,split_seq)
#==================================================================
if do_gaincor_flag == 1:#for EVN ususally
    check_sncl(cont_data, snchk, cluse,logfile)
    runtacop(cont_data,cont_data, 'CL', cluse, cluse+1, 1)
    if pol == 'I':
        gcal_app(cont_data,matxi,cluse+1,'')
    elif pol =='LR':
        gcal_app(cont_data,matxl,cluse+1,'L')
        gcal_app(cont_data,matxr,cluse+1,'R')
        #fringecal_ini(cont_data,refant, refant_candi, p_ref_cal[0],cluse+1,flagver,solint,-1,0,apfive)
    #runclcal2(cont_data,4,cluse+1,cluse+2,'ambg',1,refant,[0],p_ref_cal[0],targets)
        #run_split2(cont_data, p_ref_cal[0], cluse+2, 'LRcor', doband, bpver, flagver,split_seq)
##up: zyk++
...
if inspect_flag == 1:
    ...
    #possmplot(data[0],sources='',timer=possm_scan,gainuse=4,flagver=0,stokes='HALF',nplot=0,bpv=0,ant_use=[0],cr=0) #this will average all antennas
    #possmplot(data[0],sources='',timer=possm_scan,gainuse=4,flagver=0,stokes='HALF',nplot=1,bpv=0,ant_use=[0],cr=0)

    #if antname == 'VLBA':

if inspect_flag ==2:
    for i in range(len(pref_names)):
        possmplot(data[0],sources=pref_names[i],timer=RFIck_tran,gainuse=3,flagver=0,stokes='HALF',nplot=1,bpv=0,ant_use=[0],cr=0)
        possmplot(data[0],sources=pref_names[i],timer=RFIck_tran,gainuse=3,flagver=0,stokes='HALF',nplot=9,bpv=0,ant_use=[0],cr=1)
        possmplot(pr_data,sources=pref_names[i],timer=RFIck_tran,gainuse=4,flagver=0,stokes='HALF',nplot=1,bpv=0,ant_use=[0],cr=0)
        possmplot(pr_data,sources=pref_names[i],timer=RFIck_tran,gainuse=7,flagver=0,stokes='HALF',nplot=1,bpv=0,ant_use=[0],cr=0)
        possmplot(pr_data,sources=pref_names[i],timer=RFIck_tran,gainuse=7,flagver=0,stokes='HALF',nplot=9,bpv=0,ant_use=[0],cr=1)
    if antname != 'VLBA':
        runsnplt(data[0],inver=1,inex='TY',sources='',optype='TSYS',nplot=4,timer=[])
        possmplot(data[0],sources=p_ref_cal[0],timer=RFIck_tran,gainuse=3,flagver=0,stokes='HALF',nplot=0,bpv=0,ant_use=[0],cr=0)
...
if possm_flag==1:
    ...
    while (rep=='Y' or rep=='y' or rep=='yes'):
        if line_data2.exists():
            ...
            #runpossm(line_data2, calsource, ant_use=[0], doband, bpver, bchan, echan, gainuse, flagver=0, stokes= 'HALF', nplot)
        else:
            mprint('Using unshifted data (CVEL).',logfile)
            #runpossm(line_data, calsource, ant_use=[0], doband, bpver, bchan, echan, gainuse, flagver=0, stokes= 'HALF', nplot)
...
if ld_fr_fringe_flag == 1:
    ...
    if fr_image.exists():
        print 'removing old fr image and load again'
        fr_image.zap()
        loadfr(fr_path,fr_file,fr_nm,fr_cls,fr_dsk,antname,logfile)
    else:
        print 'load new image data'
        loadfr(fr_path,fr_file,fr_nm,fr_cls,fr_dsk,antname,logfile)


if do_fr_fringe_flag == 1:
    if cont_data.table_highver('AIPS CL')==cluse: #if no need for Gcal, copy the latest CL table
        runtacop(cont_data,cont_data, 'CL', cluse, cluse+1, 1)
    elif cont_data.table_highver('AIPS CL')>=cluse: #if done Gcal, use gcal(cluse+1)
        check_sncl(cont_data, snchk, cluse+1,logfile)
    fringecal_ini2(cont_data,refant, refant_candi, p_ref_cal[0],cluse+1,flagver,solint,-1, bpver, 1,no_rate1, dwin,rwin)
    runclcal2(cont_data,snchk+1,cluse+1,cluse+2,'AMBG',1,refant,[0],p_ref_cal[0],targets)
    if nofit_rate == 1:
    #runsncor(cont_data,sncor_timer,4)
        fringecal(cont_data, fr_image, nmaps, cluse+2, refant, refant_candi, p_ref_cal[0], solint, smodel, -1, 0, dpfour,apfive, 1,dwin,rwin)
        runclcal2(cont_data,snchk+2,cluse+2,cluse+3,'2PT',1,refant,[0],p_ref_cal[0],targets)
    else:
        fringecal(cont_data, fr_image, nmaps, cluse+2, refant, refant_candi, p_ref_cal[0], solint, smodel, -1, 0, dpfour,apfive, 0,dwin,rwin)
        runclcal2(cont_data,snchk+2,cluse+2,cluse+3,'2PT',1,refant,[0],p_ref_cal[0],targets)

if do_calib_1_flag   == 1:
    doband = -1
    check_sncl(cont_data, snchk+2, cluse+3,logfile)
    calib_1(cont_data,fr_image,'A&P',cluse+3,refant,snchk+3,doband,bpver,p_ref_cal[0],flagver,solint)
    runclcal2(cont_data,snchk+3,cluse+3,cluse+4,'2PT',1,refant,[0],p_ref_cal[0],targets)

if check_delay_rate == 1:
    #chk_sn_cl(cont_data,6,10,p_ref_cal[0],chk_trange,1)
    #chk_sn_cl(cont_data,5,9,p_ref_cal[0],chk_trange,1)
    runsnplt(pr_data,inver=cluse+4,inex='CL',sources=targets,optype='PHAS',nplot=4,timer=[])
    runsnplt(pr_data,inver=cluse+4,inex='CL',sources=targets,optype='DELA',nplot=4,timer=[])   
    runsnplt(pr_data,inver=cluse+4,inex='CL',sources=targets,optype='RATE',nplot=4,timer=[])    
    runsnplt(pr_data,inver=cluse+3,inex='CL',sources=targets,optype='PHAS',nplot=4,timer=[])
    runsnplt(pr_data,inver=cluse+3,inex='CL',sources=targets,optype='DELA',nplot=4,timer=[])    
    runsnplt(pr_data,inver=cluse+2,inex='CL',sources=targets,optype='PHAS',nplot=4,timer=[])
    runsnplt(pr_data,inver=cluse+2,inex='CL',sources=targets,optype='DELA',nplot=4,timer=[])
    runsnplt(pr_data,inver=cluse+1,inex='CL',sources=targets,optype='PHAS',nplot=4,timer=[])
    runsnplt(pr_data,inver=cluse+1,inex='CL',sources=targets,optype='DELA',nplot=4,timer=[])
    #runsnplt(pr_data,inver=cluse+4,inex='CL',sources=targets,optype='PHAS',nplot=4,timer=[])
    runsnplt(pr_data,inver=snchk+2,inex='SN',sources=targets,optype='PHAS',nplot=4,timer=[])
    runsnplt(pr_data,inver=snchk+2,inex='SN',sources=targets,optype='DELA',nplot=4,timer=[])
    runsnplt(pr_data,inver=snchk+1,inex='SN',sources=targets,optype='PHAS',nplot=4,timer=[])
    runsnplt(pr_data,inver=snchk+1,inex='SN',sources=targets,optype='DELA',nplot=4,timer=[])
    runsnplt(pr_data,inver=snchk+1,inex='SN',sources=targets,optype='RATE',nplot=4,timer=[])
    runsnplt(pr_data,inver=snchk+2,inex='SN',sources=targets,optype='RATE',nplot=4,timer=[])
    #list_sncl(pr_data,inext='CL',inver=cluse+2,sources=targets,listparm='phase')
    #list_sncl(pr_data,inext='CL',inver=cluse+2,sources=targets,listparm='delay')    
    #list_sncl(pr_data,inext='CL',inver=cluse+3,sources=targets,listparm='phase')
    #list_sncl(pr_data,inext='CL',inver=cluse+3,sources=targets,listparm='delay')
    #list_sncl(pr_data,inext='CL',inver=cluse+3,sources=targets,listparm='EL')


if split_2_flag >= 1:
    check_sncl(cont_data, snchk+3, cluse+4,logfile)
    #check_sncl(cont_data, 5, 9,logfile)
    doband=-1
    run_split2(cont_data, p_ref_cal[0], cluse+2, 'SCL9', doband, bpver, flagver,split_seq)
    run_split2(cont_data, p_ref_cal[0], cluse+3, 'SCL10', doband, bpver, flagver,split_seq)

    run_split2(cont_data, target[0], cluse+3, 'SCL10', doband, bpver, flagver,split_seq)
    run_split2(cont_data, target[0], cluse+4, 'SCL11', doband, bpver, flagver,split_seq)
    run_split2(cont_data, p_ref_cal[0], cluse+4, 'SCL11', doband, bpver, flagver,split_seq)    
#run_split2(cont_data, target[0], 11, 'SCL12', doband, bpver, flagver)
    if split_2_flag>=2:
        run_split2(cont_data, p_ref_cal[0], cluse+2, 'SCL10', doband, bpver, flagver,split_seq)
        run_split2(cont_data, p_ref_cal[0], cluse+3, 'SCL11', doband, bpver, flagver,split_seq)
...
if do_uvshift_flag ==1:
    ...
    if split3_data.exists():
        print 'Clear old split3 data'
        split3_data.clrstat()
        split3_data.zap()
    run_split3(cont_data, target[0], '4uvsh', doband, bpver, cluse+3, 0, 0)
    ...
    if shav_data.exists():
        ...
        shav_data.clrstat()
        shav_data.zap()
    run_split3(uvfix_data, target[0], 'shav', -1, 0, 0, avg_if+1, 1)
    ##run_split3(indata, target, outclass, doband, bpver, gainuse, avg(1 is keep ifs, 2 is average ifs), fittp)
###########################################################################
if ld_fr2_fringe_flag == 1:
    fr2_image=AIPSImage(fr2_nm, fr2_cls, fr2_dsk, fr2_sq)
    if fr2_image.exists():
        print 'removing old fr image and load again'
        fr2_image.zap()
    else:
        print 'load new image data'
    loadfr(fr_path,fr2_file,fr2_nm,fr2_cls,fr2_dsk,antname,logfile)


if resi_phcal_flag == 1:
    check_sncl(cont_data, snchk+3, cluse+4,logfile)
    calib_1(cont_data,fr2_image,'P',cluse+4,refant,snchk+4,-1,0,targets[1],flagver,solint)


###########################################################################
#zyk+++
import numpy as np
def p_rotate(real,imag,factor):
    if real <=0 and imag <= 0:
        phas0= np.arctan(imag/real)-np.pi
    elif real <= 0 and imag >= 0:
        phas0= np.pi+np.arctan(imag/real)
    else:
        phas0= np.arctan(imag/real)
    amp0= np.sqrt(real**2+imag**2)
    phas1=phas0 * factor
    amp1=amp0
    real_n = amp1*np.cos(phas1)
    imag_n = amp1*np.sin(phas1)
    #print phas0/np.pi*180
    return real_n, imag_n


if sn_rotate_flag == 1:
    check_sncl(cont_data, snchk+4, cluse+4,logfile)
    factor = r_factor
    runtacop(cont_data, cont_data, 'SN', snchk+4, snchk+5, 1)
    data = WAIPSUVData(outname[0], outclass[0],1, 1)
    old_tab = data.table('SN',snchk+4)
    new_tab = data.table('SN',snchk+5)
    snrow1=[]; snrow2=[]
    snrow1_new=[]
    snrow2_new=[]
    for row in old_tab:
        snrow1.append(row.real1)
        snrow2.append(row.imag1)
    for i in range(len(snrow1)):
        aa=[];bb=[]
        for j in range(4):
            a=p_rotate(snrow1[i][j],snrow2[i][j],factor)[0]
            b=p_rotate(snrow1[i][j],snrow2[i][j],factor)[1]
            aa.append(a)
            bb.append(b)
        snrow1_new.append(aa)
        snrow2_new.append(bb)
    for row in new_tab:
        row.real1 = snrow1_new[row._rownum]
        row.imag1 = snrow2_new[row._rownum]
        row.real2 = snrow1_new[row._rownum]
        row.imag2 = snrow2_new[row._rownum]
        row.update()
    old_tab.close()
    new_tab.close()
    print "Rotation done"

if sn_rotate_flag == 2: # This is for checking whether the roration is right
    real = [1,-1,1,-1]
    imag = [1,1,-1,-1]
    newr=[]
    newi=[]
    for i in range(4):
        a=p_rotate(real[i],imag[i],1)[0]
        b=p_rotate(real[i],imag[i],1)[1]
        newr.append(a)
        newi.append(b)
    print newr, newi
    print "45,135,-45,-135"
...
if app_rotate_flag == 1:
    runclcal2(cont_data,snchk+5,cluse+4,cluse+5,'2PT',1,refant,[0],targets[1],targets)
    runsnplt(cont_data,inver=snchk+4,inex='SN',sources=targets,optype='PHAS',nplot=4,timer=[])
    runsnplt(cont_data,inver=snchk+5,inex='SN',sources=targets,optype='PHAS',nplot=4,timer=[])
    runsnplt(cont_data,inver=cluse+5,inex='CL',sources=targets,optype='PHAS',nplot=4,timer=[])

if splt_3_flag == 1:
    doband,bpver=[-1,0]
    run_split2(cont_data, targets[0], cluse+5, 'S12-RO', doband, bpver, flagver,split_seq)   

...
# ============================================================================
AIPS_NUMBER = 310  # AIPS user number
...
file_name = 'bl307d_x.idifits'  # Better use obs_code.idifits as name
...
reference_antenna = 3
search_antennas = [9, 5, 0]
scan_for_fringe = [0,18,39,0,0,18,41,0]
...
# each sublist corresponds to a separate flagging round, should be consistent with 
# other flagging parameters throughout this part (before Special quack flagging)
...
# Time flagging
fgtimer=[[0,16,10,0,0,16,11,50],[0]]
# Time ranges to flag
...
sp_quack_flag = 1  # 0 mean no special quack flagging, 1 means enable special quack flagging and using the following parameters.
...
# ============================================================================
ant_gan_cal = 1  # Enable antenna gain calibration
pol = 'I'  # Polarization: 'I' for Stokes I only, 'LR' for both hands
# Gain correction factors per antenna per source
matxi=[[1.1,5.5,1.0,2.0,0.9,1.0,0.9,0.9,1.0,1.0],
       [1.1,5.5,0.9,2.0,0.9,1.0,0.9,0.9,1.0,1.0],
       [1.1,5.6,1.0,1.9,1.0,1.0,0.9,0.9,1.0,1.0],
       [1.1,5.9,0.9,2.0,0.9,1.0,0.9,0.9,0.9,1.0]]
...
# ============================================================================
man_fr_file = ['J1905-v2-gsmod1.fits']  # Manual model files (when auto_mapping=0)
...
final_cl_ver = 9  # currently, 9 means no-smo versino and 10 for smo version
# ============================================================================
...
rash = [-2.56e-3]  # RA shift (no need to multiply by cos(dec))
decsh = [-3.94e-4]  # Dec shift
